        ...
        <table className="w-full">
          <thead>
            <tr className="text-left border-b border-mercury-border-light">
              <th className="px-4 py-2 text-xs text-mercury-text-secondary font-medium">
                Date
              </th>
              <th className="px-4 py-2 text-xs text-mercury-text-secondary font-medium">
                To/From
            ...
            {transactions.map((tx, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-xs flex items-center">
                  {tx.name.startsWith('From') && (
                    <ArrowDown className="h-3 w-3 mr-1 text-mercury-money-in" />
                  )}
                  {tx.name.startsWith('To') && (
                    <ArrowUp className="h-3 w-3 mr-1 text-mercury-money-out" />
                  )}
                  {tx.status === 'failed' ? (
